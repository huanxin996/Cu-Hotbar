# CuHotbar V1.0.7 for Casualties: Unknown
...
- **Auto reload**: on by default. When the firearm in your main hand is short of ammo (no magazine / direct-feed magazine not full / racked but chamber empty), the mod scans your inventory for matching `ammoType` and loads one round / magazine; full magazines are preferred. No keypress and no hand swap.
...
After launch, a "CuHotbar" button is injected on the main menu; in-game, opening the pause panel with ESC also shows a launcher button at the bottom-right. When CuSaveManager is present, settings move into its "CuHotbar" tab instead.
...
| Switch keys | Per-slot switch key, use-held-item key, scroll switch, auto-refill, drop warning, safe quick use, auto reload |
| Position | Screen anchor X/Y, pixel offset X/Y, reset position |
| Hotkeys | Open / close the settings panel |
| Misc | Show mod logs in game console, accept update notifications, UI language (auto / Chinese / English) |
...
- **CuSaveManager**: when present, this mod registers its settings as a tab via CuSaveManager's extension point for a unified style; when absent or on a version without the extension point, it falls back to a standalone main-menu button + in-game launcher button + its own settings window.
